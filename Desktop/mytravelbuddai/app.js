...
const API_CONFIG = {
    GEMINI_API_KEY: 'AIzaSyB3IPnR7Xq_e9EdMM7dVLiYTR4-F0yScpk',
    UNSPLASH_API_KEY: '8OqCoB6ptQNpzEgZSdqg2ogyoen_2zARwfb_ItErd1Y',
    WEATHER_API_KEY: '8bceb06cc2044ece885115050242411'
};
...
// Weather and Image fetch functions 
async function fetchWeather(city) {
    const response = await fetch(`https://api.weatherapi.com/v1/current.json?key=${API_CONFIG.WEATHER_API_KEY}&q=${city}`);
    if (!response.ok) throw new Error('Weather data fetch failed');
    return response.json();
}

async function fetchCityImage(city) {
    const response = await fetch(`https://api.unsplash.com/search/photos?query=${city}&client_id=${API_CONFIG.UNSPLASH_API_KEY}`);
...
async function generateEventSuggestions(city, weatherCondition) {
    try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1/models/gemini-pro:generateContent?key=${API_CONFIG.GEMINI_API_KEY}`, {
            method: 'POST',
            headers: {
